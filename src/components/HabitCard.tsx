...
import { Check, Flame, Snowflake, MoreVertical, Info } from "lucide-react"
...
export default function HabitCard({ habit, onDone, onEdit }: Props) {
  ...
  return (
    <div className="bg-yt-bg animate-pop-in" style={{ borderBottom: "1px solid rgba(0,0,0,0.08)" }}>
      ...
      <div className="relative flex h-[148px] w-full items-center justify-center" style={{ background: col.thumb }}>
        ...
        {/* streak badge (top-left, like YouTube view count badge) */}
        {streak > 0 && (
          <div
      ...
      {/* ── metadata (YouTube video info style) ── */}
      <div className="flex items-start gap-3 px-3 pt-3 pb-1">
        {/* channel icon = small emoji circle */}
        <div
      ...
      {/* ── action bar (like / dislike / share style) ── */}
      <div className="flex items-center gap-0 px-3 pb-3 pt-1">
        ...
        {/* まあいっか = Dislike/skip button */}
        {!isDone && (
          <div className="relative">
            <button
              onClick={() => useFreeze(habit.id)}
        ...
        {/* 残トークン表示 */}
        {!isDone && (
          <span className="text-[12px]" style={{ color: "#606060" }}>
            残{habit.freezeTokens}回
          </span>
        )}
      </div>
      ...
      {showFreezeInfo && (
        <div
          className="mx-3 mb-3 rounded-yt p-3 text-[13px] leading-relaxed"
        ...
        >
          ...
          <p className="mt-1">
            「どうしても無理だった日」に使えるサボり免罪符です。
            押すと<strong>連続記録（ストリーク）が途切れません</strong>。
          </p>
          <p className="mt-1">
            サボったからといって責める必要はありません。
            完璧よりも「続けること」のほうが大切です。
          </p>
          <p className="mt-1">
            各習慣に<strong>{habit.freezeTokens}回分</strong>残っています。使い切っても毎月3回補充されます。
          </p>
        </div>
      )}
    </div>
